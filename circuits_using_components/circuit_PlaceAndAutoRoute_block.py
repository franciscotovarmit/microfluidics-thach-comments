...
class CircuitOfBlocks(microfluidics.PlaceAndAutoRoute):
    ...
    block_with_tee = i3.ChildCellListProperty() #i3.ChildCellListProperty()  #  Generating traps with Tee from Child Cell List Property
    ...
    def _default_block_with_tee(self):  # Generating traps from Child Cell List Property
        ...
        my_block = Block()
        my_block.Layout(block_length=300.0)
        return [BlockWithTees(name="blk_w_tee_{}".format(cnt),
                             block=my_block,
                             tee=tee1,
                             )
                for cnt in range(self.n_blocks_x*self.n_blocks_y)]
