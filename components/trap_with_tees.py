...
if __name__ == "__main__":
    ...
    a = trap.Layout(cell_trap_length= 300.0) #(cell_trap_length= 300.0)
    a.visualize_2d()
    tee = TeeSimple()
    tee.Layout(tee_length=100)

    myTrap_withTees= TrapWithTees(
                            trap=trap,
                            tee=tee)
    layout = myTrap_withTees.Layout()
    ...
    simul.procedure.run(case_name='one_trap_with_tees', use_existing_mesh=False, interactive=True)
